...
public class GetIntructoresQuery
{
    ...
    internal class GetInstructoresQueryHandler : IRequestHandler<GetInstructoresQueryRequest, Result<PagedList<InstructorResponse>>>
    {
        ...
        public GetInstructoresQueryHandler(MasterNet9DbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public async Task<Result<PagedList<InstructorResponse>>> Handle(GetInstructoresQueryRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                Expression<Func<Instructor, object>>? orderBySelector = request.InstructorRequest.OrderBy.ToLower() switch
                {
            ...
            var pagination = await PagedList<InstructorResponse>
                .CreateAsync(instructoresQuery, request.InstructorRequest.PageNumber, request.InstructorRequest.PageSize);
